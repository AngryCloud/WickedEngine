...
#include "GodClient/WickedGodClientPanelWindowFactory.h"
#include "wiGUI.h"
...
using DmoGodClientPanelWindowFactory = dmo::wicked::DmoGodClientPanelWindowFactory;
...
public:
    void Load() override
    {
        ...
        if (!m_godClientPanelFactory)
            m_godClientPanelFactory = std::make_unique<DmoGodClientPanelWindowFactory>(
                GetGUI(),
                []() noexcept
                {
                    return XMFLOAT2(WickedUI::GetFrontDoorLogicalWidth(),
                                    WickedUI::GetFrontDoorLogicalHeight());
                });
